...
def twse_crawler(year=2019, mm=5, dd=22, stockNo=2330):
    ...
    url = 'https://www.twse.com.tw/exchangeReport/STOCK_DAY?response=json&date=20250122&stockNo=3231'
    ...
    stock_info
    # stock_info[2]     title: 108年05月 2330 台積電 各日成交資訊
    # stock_info[3]     fields: ['日期', '成交股數', '成交金額', '開盤價', '最高價', '最低價', '收盤價', '漲跌價差', '成交筆數', '註記']
    

    import pandas as pd
    import numpy as np

    stock_price = pd.DataFrame(stock_info[4])   #data: 個股每日成交資訊資料
    stock_price
...
twse_crawler(year=2025, mm=1, dd=22, stockNo=3231)
